...
// Minimal drop-in replacement for react-router's <Link>.
// We keep it extremely small to avoid a sweeping JSX rewrite.
function Link(props: { to: string; style?: React.CSSProperties; children: React.ReactNode }) {
  return (
...
// Next.js wrappers pass these in (replaces react-router params + navigation).
export type MapPageRouteProps =
  | { kind: 'root' }
  | { kind: 'ping'; id: string }
...
function MapPage({ route, onNavigate }: MapPageProps) {
  const isShareRoute = route.kind === 'share';
  const id = route.kind === 'ping' ? route.id : undefined;
  const shareToken = route.kind === 'share' ? route.shareToken : undefined;
  ...
  const { isLoaded } = useJsApiLoader({
    ...
    // Next.js exposes browser env vars via NEXT_PUBLIC_*
  ...
  const fetchHotspotById = async (hotspotId: string) => {
    try {
      ...
      // If location is already available, auto-enable proximity detection
      // Otherwise, user must click "Enable GPS" to enable
                            ...
                            <div className="modal-section modal-prize">
                              ...
                              {selectedHotspot.claimType === 'proximity' && isWithinProximityRadius && !id ? (
                                <button
                                  ...
                                  onClick={() => navigateTo(`/ping/${selectedHotspot.id}`)}
                                >
                                  CHECK PROXIMITY
                                </button>
                              ) : (
